...
_config = (configFile >> "CfgSettings" >> "X39" >> "ServerConfig");
for "_i" from 0 to ((count _config) - 1) do
{
	...
	{
		if(X39_MedSys_var_varNames find _name == -1) then
		{
			diag_log format["serverconfig error found! Variable '%1' is not defined by XMedSys!", _name];
		}
		else
		{
